...
public extension Popup {
    ...
    /// Displays the popup. Stacks previous one
    func showAndStackAndReturn() -> some Popup { PopupManager.showAndStack(AnyPopup<Config>(self)); return self }

    /// Displays the popup. Closes previous one
    func showAndReplace() { PopupManager.showAndReplace(AnyPopup<Config>(self)) }
    /// Displays the popup. Closes previous one
    func showAndReplaceAndReturn() -> some Popup { PopupManager.showAndReplace(AnyPopup<Config>(self)); return self }
